...
#' differentialRegPlot
#'
#' \code{differentialRegPlot} Function to be applied to more and RegulationPerCondition functions outputs. Creates an upset plot to analyze the differential regulators across conditions and omics.
#' 
#' @param output MORE object obtained from applying \link{more} function. 
#' @param outputRegpcond Object generated by the function \link{RegulationPerCondition} when applied to a \link{more} object. 
#' 
#' @return UpSet plot of the differential regulators across conditions and omics induced from more.
#' @export

differentialRegPlot = function(output, outputRegpcond){
  groups = unique(output$arguments$groups)
  omics = names(output$arguments$regulatoryData)
  ...
  for (i in (1:length(groups))){
    for(j in (1:length(omics))){
      temp =outputRegpcond[outputRegpcond[,pos+i-1]!=0,]
      regulators = temp[temp$omic==omics[j],]$regulator
      data_list[[paste(groups[i], omics[j], sep='_')]] = regulators
    }
  }
  ...
  # Convert the data to the format needed to create the upset plot per group
  reg_omic = unique(df[c("regulator", "omic")])
  for (g in groups) {
    group_subset = df[df$group == g, ]
    present = paste(group_subset$regulator, group_subset$omic)
    reg_omic[[g]] = as.integer(paste(reg_omic$regulator, reg_omic$omic) %in% present)
  }
  
  #Ask for biostatomics colors
  num_unique = length(groups)+1
  color_palette = colorbiostat(num_unique)
  custom_colors = setNames(color_palette[-1], groups)
  
  num_unique = length(groups)+length(omics)+1
  color_palette = colorbiostat(num_unique)
  omic_colors = setNames(color_palette[length(groups)+2:num_unique], omics)
  
  queries = lapply(groups, function(cond) {
  ...
  ComplexUpset::upset(
    ...
    base_annotations=list(
      'Omic with counts'=(
        ...
        + scale_fill_manual(values=omic_colors, name='Omic')
        + ylab('Proportion of regulators')
        + theme(
          panel.grid.major=element_blank(),
          panel.grid.minor=element_blank(),
    ...
    stripes=ComplexUpset::upset_stripes(
      geom=geom_segment(linewidth=12),
    ...
    wrap=TRUE
  ) + ggtitle('Differential regulators across conditions')
...
networkMORE <- function(outputRegpcond, cytoscape = TRUE, group1 = NULL, group2 = NULL, pc = 0, pathway= NULL, annotation = NULL, arrows=FALSE, save=FALSE) {
  ...
  create_graph <- function(df,pc, arrows = FALSE) {
    ...
    temp_labels <- mapply(function(id, omic) {
      sub(paste0("^", omic, "_"), "", id)
    }, nodes$id, nodes$omic)
    
    label_counts <- ave(seq_len(nrow(nodes)), temp_labels, FUN = length)
    nodes$label <- ifelse(label_counts == 2, temp_labels, nodes$id)
    
    final_counts <- ave(seq_len(nrow(nodes)), nodes$label, FUN = length)
    nodes <- nodes[!(final_counts == 2 & nodes$omic == "targetF"), ]
